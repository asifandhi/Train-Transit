...
export const cancelBooking = asyncHandler(async (req, res) => {
  ...
  if (booking.bookingStatus === 'cancelled') throw new apiError(400, 'Booking already cancelled')
  if (booking.bookingStatus === 'completed') throw new apiError(400, 'Cannot cancel a completed journey')
  ...
  const cancellation = await Cancellation.create({
    ...
    refundStatus: booking.paymentStatus === 'paid' ? 'pending' : 'pending',
  ...
  const incUpdate = {}
  if (counts.confirmed > 0) incUpdate[`availableSeats.${cls}`] = counts.confirmed
...
export const getRefundStatus = asyncHandler(async (req, res) => {
  ...
  if (!cancellation) throw new apiError(404, 'No cancellation record found for this PNR')

  const isOwner = cancellation.user._id.toString() === req.user._id.toString()
  const isAdmin = req.user.role === 'admin'
  if (!isOwner && !isAdmin) throw new apiError(403, 'Not authorized to view this refund')
